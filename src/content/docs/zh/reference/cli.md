...
- 你希望使用 `login`、`logout`、`whoami`、`auth status` 这类本地 auth 辅助命令
...
- Pro 付费交接：`agent-analytics upgrade-link --detached`
...
在托管 agent runtime 中，home 目录下的配置不一定可靠持久化。登录前请指定一个持久目录：
...
- 账户与认证：`login`、`logout`、`whoami`、`auth status`、`upgrade-link`
