...
如果你测试的是注册 CTA，请先区分清楚一件事：CTA 点击本身通常不等于规范意义上的 `signup`。在大多数产品里，`signup` 应该在账户真正创建时触发，通常放在服务端。如果你也想测点击这一步，请单独记录 `signup_started` 或 `signup_cta_clicked`。
...
```text
确认完成注册时已经有一个规范的 signup 事件。如果这个 CTA 只是启动流程，就为按钮点击单独添加 signup_started 或 signup_cta_clicked 事件，并保留原来的 signup 完成事件不变。
```
...
```text
告诉我怎样在本地强制显示每个变体，以便我 QA 两个版本，然后确认规范的 signup 事件仍然能正确落地，同时任何中间 CTA 事件在每个版本里也都能正常触发。
```
...
- 在目标事件还没真正埋好之前就先创建实验。
- 把认证前的 CTA 点击当成 `signup`，而真实转化其实发生在后面的认证或账户创建步骤里。
- 上线前没有先 QA 强制变体。
